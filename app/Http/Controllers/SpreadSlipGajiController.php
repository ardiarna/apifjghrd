...
class SpreadSlipGajiController extends Controller
{
    ...
    public function cetak($tahun, $bulan, $jenis, $area) {
        ...
        foreach ($dataDetails as $i => $d) {
            if ($i % 2 == 0) {
                ...
                $start = $sheetIndex * 2 + 1;
                $end = $start + 1;
                $startLabel = str_pad($start, 2, "0", STR_PAD_LEFT);
    ...
    public function perKaryawan($karyawan_id, $tahun, $bulans) {
        ...
        $arrBulan = ['Desember', 'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];
        $arrBulanPendek = [1 => 'Jan', 2 => 'Feb', 3 => 'Mar', 4 => 'Apr', 5 => 'Mei', 6 => 'Jun', 7 => 'Jul', 8 => 'Agu', 9 => 'Sep', 10 => 'Okt', 11 => 'Nov', 12 => 'Des'];
        $px = 10.7/64;      //100 -> 100;
        $spreadsheet = new Spreadsheet();
        $spreadsheet->getDefaultStyle()->getFont()->setName('Ebrima')->setSize(10)->setBold(TRUE);
        ...
        foreach ($chunks as $pair) {
            ...
            $spreadsheet->setActiveSheetIndex($sheetIndex);
            $si->setShowGridlines(false);
            $bulanLabels = array_map(function($d) { return $d->bulan; }, $pair);
            sort($bulanLabels);
            $sheetName = implode(' & ', array_map(function($b) use ($arrBulanPendek) {
                return $arrBulanPendek[intval($b)];
            }, $bulanLabels));
        ...
        foreach ($spreadsheet->getAllSheets() as $sheet) {
            ...
            $sheet->getStyle('H1:H'.$lastRow)->getNumberFormat()->setFormatCode('#,##0');
            $sheet->getStyle('J1:K'.$lastRow)->getNumberFormat()->setFormatCode('#,##0');
            $sheet->getStyle('P1:P'.$lastRow)->getNumberFormat()->setFormatCode('#,##0');
        }
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
