...
class SpreadKaryawanCustomController extends Controller
{
    ...
    public function alamatEngineeringDept() {
        ...
        $si->setTitle('ALAMAT ENGINEERING');

        // Fetch Data
        $all = $this->repoKaryawan->findAll(['aktif' => 'Y']);
        $dataKaryawan = [];
        foreach ($all as $d) {
            if ($d->divisi && ($d->divisi->kode == 'EN' || stripos($d->divisi->nama, 'ENGINEERING') !== false)) {
                $dataKaryawan[] = $d;
            }
        }
        ...
        $si->setCellValue('A2', 'DATA/ALAMAT ENGINEERING DEPT. (TEKNISI)');
        $si->mergeCells('A2:G2');
        ...
        header('Content-Disposition: attachment;filename="ALAMAT_ENGINEERING_DEPT.xlsx"');
    ...
    public function dataEngineeringDept() {
        $spreadsheet = new Spreadsheet();
        $spreadsheet->getDefaultStyle()->getFont()->setName('Calibri')->setSize(10)->setBold(TRUE);
        
        $si = $spreadsheet->getActiveSheet();
        $si->setShowGridlines(false);
        $si->setTitle('DATA ENGINEERING');
        ...
        $si->setCellValue('A2', 'LIST DIVISON ENGINEERING');
        $si->mergeCells('A2:I2');
        $si->setCellValue('A3', $areaString);
        $si->mergeCells('A3:I3');
        ...
        $widths = ['A'=>4.55, 'B'=>38.00, 'C'=>18.00, 'D'=>18.00, 'E'=>11.07, 'F'=>15.00, 'G'=>39.95, 'H'=>58.04, 'I'=>40.00];
        foreach ($widths as $col => $width) {
            $si->getColumnDimension($col)->setWidth($width);
        }
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="DATA_ENGINEERING_DEPT.xlsx"');
        header('Cache-Control: max-age=0');
        header('Cache-Control: max-age=1');
        header('Expires: Mon, 26 Jul 1997 05:00:00 GMT');
        header('Last-Modified: ' . gmdate('D, d M Y H:i:s') . ' GMT');
