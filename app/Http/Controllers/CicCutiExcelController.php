...
class CicCutiExcelController extends Controller
{
    ...
    public function jadwal($tahun)
    {
            ...
            $drawTable = function($title, $isManajemen) use (&$sheet, &$row, $details, $m, $tahun, $daysInMonth, $arrkol, $bulanIndo, $holidayDatesRed, $holidayDatesGreen) {
                ...
                foreach ($details as $staf => $areas) {
                    ...
                    foreach($cutiDatesMonth as $cd) {
                        ...
                        $kat = $cd->cicCutiDetail->kategori ?? '';
                        if($kat == 'UNPAID') {
                            $cntUnpaid++;
                        }
                        elseif($kat == 'GANTI_HARI_LIBUR') {
                            $cntGanti++;
                            $ket = trim($cd->cicCutiDetail->keterangan ?? '');
        ...
        return $this->downloadExcel($spreadsheet, "JADWAL_CUTI_CIC_$tahun.xlsx");
    }
    ...
    public function form($id)
    {
        ...
        $tahunanRows = [
            ...
            21 => [($cicCuti->jenis_form == 'IJIN' ? 'Ijin Yang Akan Diambil' : 'Cuti Yang Akan Diambil'),                     'G', $akanDiambil,  'Hari'],
