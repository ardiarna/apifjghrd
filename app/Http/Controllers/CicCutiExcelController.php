...
class CicCutiExcelController extends Controller
{
                ...
                foreach ($details as $staf => $areas) {
                    ...
                    foreach ($areas as $area => $karyawansGrp) {
                        ...
                        foreach ($karyawansGrp as $k) {
                            ...
                            $cutiDatesMonth = \App\Models\CicCutiDate::with('cutiDetail')
                                ->whereMonth('tanggal', $m)
                                ->whereHas('cutiDetail.cicCuti', function($q) use ($k, $tahun) {
                    ...
                    $diambil = \App\Models\CicCutiDate::whereHas('cutiDetail', function($q) use ($k, $tahun) {
    ...
    public function listCuti($tahunAwal, $tahunAkhir)
    {
        ...
        // Freeze Panes (same as data cicKaryawan, freeze pane on C7)
        $sheet->freezePane('C7');

        $row = 7;
        $idx = 1;

        // 3. Urutan cicKaryawan harus sama persis seperti di excel data cicKaryawan
        foreach ($details as $staf => $areas) {
            if($staf == 'N') {
                $sheet->setCellValue('B'.$row, 'NON STAF :');
                $sheet->getStyle('B'.$row)->getAlignment()->setHorizontal('center');
            ...
            foreach ($areas as $area => $cic_karyawans) {
                if($staf == 'Y') {
                    $sheet->setCellValue('B'.$row, $area.' :');
                    $sheet->getStyle('B'.$row)->getAlignment()->setHorizontal('center');
                    $sheet->getStyle('B'.$row)->getFont()->getColor()->setARGB('0000FF');
                ...
                foreach ($cic_karyawans as $k) {
                    ...
                    for ($i = 0; $i < $maxRows; $i++) {
                        if ($i == 0) {
                            ...
                            for($m=1; $m<=12; $m++) {
                                $diambilBulan = \App\Models\CicCutiDate::whereHas('cutiDetail', function($q) use ($k, $tahun) {
                            ...
                            $cutiBersama = \App\Models\CicCutiDate::whereHas('cutiDetail', function($q) use ($k, $tahun) {
                            ...
                            $jmlIjin = \App\Models\CicCutiDate::whereHas('cutiDetail', function($q) use ($k, $tahun) {
    ...
    public function form($id)
    {
        ...
        $cicCuti = CicCuti::with([
            'cicKaryawan.jabatan',
            'cicKaryawan.divisi',
            'cicKaryawan.statusKerja',
            'cicKaryawan.area',
            'details.dates',
            'details.cicJenisCutiKhusus',
        ])->find($id);
        ...
        foreach ($cicCuti->details as $det) {
            ...
            if ($det->kategori === 'KHUSUS') {
                $lamaKhusus += $lama;
                if ($det->cicJenisCutiKhusus && strtolower($det->cicJenisCutiKhusus->satuan) === 'bulan') {
        ...
        // Tanggal pengambilan cicCuti (format panjang, deduplikasi)
        $tanggalInput = '-';
        if (count($allDates) > 0) {
            sort($allDates);
    ...
    public function tanpaPotongan($tahunAwal, $tahunAkhir)
    {
        ...
        $karyawans_raw = CicKaryawan::with('jabatan', 'area')
            ->where('aktif', 'Y')
            ->whereHas('cic_cutis', function($q) use ($tahun) {
                ...
                foreach ($cic_karyawans as $k) {
                    ...
                    for ($i = 0; $i < $maxRows; $i++) {
                        if ($i == 0) {
                            ...
                            $totalBulan = 0;
                            foreach ($detailsKet as $det) {
                                $sat = strtolower($det->cicJenisCutiKhusus->satuan ?? 'hari');
                ...
                foreach ($cic_karyawans as $k) {
                    ...
                    for ($i = 0; $i < $maxRows; $i++) {
                        if ($i == 0) {
                            ...
                            $totalJumlah = 0;
                            for($m=1; $m<=12; $m++) {
                                $jml = \App\Models\CicCutiDate::whereHas('cutiDetail', function($q) use ($k, $tahun) {
